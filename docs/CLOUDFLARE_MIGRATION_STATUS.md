...
| D1 commerce correctness | First authenticated API slice complete & hardened; staging deployment pending | D1 commerce schema (`0002_commerce.sql`) implements inventory, cart, atomic reservations with `RAISE(ABORT)` rollback, orders, order lines, coupons, and cancellation trigger guard (`trg_cancel_order`). PostgreSQL products exported to D1 seed migration (`0003_seed_inventory.sql`) without invented stock or concept leakage. Commerce API (`cloudflare/worker/src/commerce.py`) hardened against auth bypass (missing JWT secret rejected with 503, dev header guarded by `ENVIRONMENT == 'test'`), validates delivery address ownership and 6-digit pincode, applies coupon rules (`MILTERRA10`, `FARMER50`), verifies fingerprint match on concurrent idempotency races, prevents double cancellation, and implements payment capabilities/link matching Flutter client contracts. Local basket bidirectional reconciliation (`cart_provider.dart`) and storage transparency (`isPersisted`) implemented. |
...
4. [Completed locally] Hardened D1 commerce slice: current price/stock, customer cart (CRUD with PUT item quantity), authoritative quote (with address ownership & coupon logic), atomic D1 batch reservation (with `RAISE(ABORT)` rollback), idempotent order creation with fingerprint conflict rejection, cancellation eligibility & trigger-guarded stock restoration, payment capabilities and payment link endpoints, and real PostgreSQL product inventory exported to D1 seed migration without stock invention.
