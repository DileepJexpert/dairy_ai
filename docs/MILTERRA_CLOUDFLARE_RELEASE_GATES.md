...
1. Restore Wrangler account access. Create **separate** staging Pages project, Worker, D1 database and any required R2 buckets. Copy `cloudflare/worker/wrangler.staging.example.toml` to a private config, replace the deliberately invalid D1 ID and storefront origin, and set required secrets with Wrangler. Keep production IDs and secrets out of Git.
